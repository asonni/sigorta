...
class SalesAPIController {
  salesIndex(req, res) {
    ...
    .then(clients => {
      ...
      return service.fetchAllSales(start, end, clientsIds, dateType)
    })
    .then(sales => {
      return res.status(200).json({ sales, clients: c, totalPriceSum: _.sumBy(sales, 'totalPrice'), totalPriceAfterDiscountSum: _.sumBy(sales, 'totalPriceAfterDiscount') })
    })
    .catch(e => {
